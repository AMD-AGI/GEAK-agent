...
class LLMBrain:
    ...
    def _init_client(self):
        ...
        if not HAS_OPENAI:
            self._log("⚠️  OpenAI library not available. LLM features disabled.")
            return
        ...
        if not api_key:
            self._log("=" * 60)
            self._log("⚠️  MINI_KERNEL_API_KEY environment variable not set!")
            self._log("")
        ...
        self._log("🧠 LLM Brain initialized")
    ...
    def _log_llm_call(self, operation: str, prompt_preview: str = None):
        ...
        self._log(f"")
        self._log(f"  ┌─ LLM Call #{self.call_count} [{timestamp}]")
        self._log(f"  │  Operation: {operation}")
        if prompt_preview:
            preview = prompt_preview[:100].replace('\n', ' ')
            self._log(f"  │  Prompt: {preview}...")
    ...
    def _log_llm_response(self, response_preview: str = None, tokens: int = 0):
        ...
        self._log(f"  └─ Total calls: {self.call_count}")
    ...
    def analyze_profiler_output(self, 
        ...
        self._log("")
        self._log("=" * 60)
        self._log("  🔍 BOTTLENECK ANALYSIS (LLM)")
        self._log("=" * 60)
...
ONLY output the JSON, nothing else."""
        ...
        try:
            ...
            self._log(f"  ⚠️  Analysis failed: {e}")
            return {
                "primary_bottleneck": "balanced",
                "confidence": 0.0,
                "analysis": f"Error: {e}",
    ...
    def generate_optimization(self,
        ...
        self._log("")
        self._log(f"  🧬 GENERATING OPTIMIZATION (Gen {gen_num})")
...
ONLY output the code block, nothing else."""
        ...
        try:
            ...
            self._log(f"  ⚠️  Generation failed: {e}")
            return OptimizationCandidate(
...
""",
    ...
    def mutate_optimization(self,
        ...
        """
        self._log("")
        self._log(f"  🔀 MUTATION: {candidate.strategy_name}")
...
```"""
        ...
        try:
            ...
            self._log(f"     ⚠️  Mutation failed: {e}")
...
```"""
        ...
        try:
            ...
            self._log(f"     ⚠️  Crossover failed: {e}")
...
class OpenEvolveBrain:
    ...
    def evolve(self,
        ...
        self._log("")
        self._log("╔" + "═" * 58 + "╗")
        self._log("║" + "  🧬 OPENEVOLVE OPTIMIZATION".center(58) + "║")
        self._log("╚" + "═" * 58 + "╝")
        self._log("")
        ...
        self._log("")
        self._log(f"  Target:")
        self._log(f"    Baseline:        {baseline_time_us:.2f} μs")
        self._log(f"    Target speedup:  {target_fitness:.2f}x")
        self._log(f"    Target time:     {baseline_time_us/target_fitness:.2f} μs")
        self._log("")
        ...
        for i in range(self.population_size):
            ...
            try:
                ...
                self._log(f"     ✓ Fitness: {candidate.fitness:.2f}x speedup")
            except Exception as e:
            ...
            if candidate.fitness >= target_fitness:
                self._log(f"\n  🎯 TARGET ACHIEVED in initialization!")
        ...
        for gen in range(self.generations):
            self._log("")
            self._log(f"  ┌─ Generation {gen + 1}/{self.generations} " + "─" * 35)
            self._log(f"  │  Best so far: {best_ever.strategy_name} ({best_ever.fitness:.2f}x)")
            ...
            for i in range(min(self.elitism, len(sorted_pop))):
                ...
                self._log(f"  │  [Elite] {sorted_pop[i].strategy_name} ({sorted_pop[i].fitness:.2f}x)")
            ...
            while len(new_population) < self.population_size:
                ...
                if op < self.crossover_rate and len(population) >= 2:
                    ...
                    self._log(f"  │")
                    self._log(f"  │  [Candidate {candidate_num}] Crossover")
                    ...
                    self._log(f"  │")
                    self._log(f"  │  [Candidate {candidate_num}] Mutation")
                    ...
                    self._log(f"  │")
                    self._log(f"  │  [Candidate {candidate_num}] New Generation")
                ...
                try:
                    candidate.fitness = evaluate_fn(candidate.code)
                    self._log(f"  │     ✓ Fitness: {candidate.fitness:.2f}x")
                except Exception as e:
                    candidate.fitness = 0.0
                    self._log(f"  │     ✗ Failed: {str(e)[:50]}")
            ...
            self._log(f"  │")
            self._log(f"  │  Generation Summary:")
            self._log(f"  │    Best:    {gen_best.fitness:.2f}x ({gen_best.strategy_name})")
            self._log(f"  │    Average: {gen_avg:.2f}x")
            self._log(f"  └─" + "─" * 50)
    ...
    def _print_final_report(self, best: OptimizationCandidate, baseline_us: float):
        ...
        self._log("")
        self._log("╔" + "═" * 58 + "╗")
        self._log("║" + "  🏁 OPTIMIZATION COMPLETE".center(58) + "║")
        self._log("╚" + "═" * 58 + "╝")
        self._log("")
        ...
        if best.parent_strategies:
            self._log(f"  Lineage: {' → '.join(best.parent_strategies)} → {best.strategy_name}")
            self._log("")
