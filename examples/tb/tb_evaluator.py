...
class BulletproofTritonEvaluator:
    ...
    GOLDEN_DATA_PATH = os.path.join("/shared-aig/vinay/code/main/tb_eval/data/ROCm/data/performance/golden_results")
    ...
    def evaluate(self, program_text: str, kernel_evaluator_path: str, gpu_id: int=0) -> Dict[str, Any]:
        ...
        try:
            ...
            cmd = ["tb_eval", "eval", "-f", f'{program_text}', "-o", 'tbg_results', "--dataset", 'tbg', "-c" ]
            ...
            rocm_runs_file = os.path.join(ext_removed, "tbg_results")
            ...
            bname = os.path.basename(program_text)
            benchmark_file = os.path.join(ext_removed, "exec", "gen_perf", bname.replace(".py", ".json"))
            baseline_file = os.path.join(tb_eval.constants.TBG_PERF_GOLD_DATA_ROOT, bname.replace(".py", ".json"))
            ...
            if not os.path.exists(benchmark_file):
                ...
                with open(benchmark_file, 'r') as f:
                    benchmark_data = json.load(f) #f.read().strip()
                ...
                with open(baseline_file, 'r') as f:
                    ...
                    for entry in baseline_data:
                        baseline_ms += float(entry["ms"])

                benchmark = baseline_ms / global_ms if global_ms > 0 else None
            ...
            if not os.path.exists(benchmark_file):
                ...
                baseline_fname = os.path.basename(benchmark_file)
                with open(benchmark_file, 'r') as f:
                ...
                for item in json_data:
                    params = item.get("input_size", [])
                    latency = item.get("ms", None)
                    if params and latency is not None:
                        benchmark_params += "For parameter shapes: "
                        params_str = ";".join(f"{v}" for v in params)
