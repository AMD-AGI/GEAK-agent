...
class TritonBench:
    ...
    def write_perf_file(self, input_folder_path, results_path, tmp_dir):
        ...
        for file in input_file_list:
            if file[-3:] == ".py":
                op = file[:-3]
                match = re.match(r"^([a-zA-Z0-9_]+?)(?:_\d+)?\.py$", file)
                if match:
                    perf_op = match.group(1)
                perf_file_name = perf_op + "_perf.py"
                op_file_name = op + "_perf.py"
                ...
                with open(os.path.join(self.golden_metrics_folder, perf_file_name), "r") as f:
                    ...
                    for line in lines:
                        ...
                        line = line.replace(f"from TritonBench_v1.{perf_op}", f"from {op}")
                        line = line.replace(f"super().__init__('{perf_op}'", f"super().__init__('{op}'")
                ...
                with open(os.path.join(tmp_dir, op_file_name), "w") as f:
            ...
            for idx, script in enumerate(scripts):
                ...
                try:
                    ...
                    if result.returncode==0:
                        ...
                        tqdm.write(f"❌ Error {idx+1}/{total_scripts}: {os.path.basename(script)} {result.stderr}")
                except subprocess.TimeoutExpired:
    ...
    def calculate(self, path_gen, path_ref=None):
        ...
        # TODO: there is problem with efficiency calculation
        efficiency = max(round(max(get_gbs(data_gen)) * 100 / 3276.8, 4), round(max(get_tflops(data_gen)) * 100 / 362.1, 4))
        # efficiency1 = max(round(max(get_gbs(data_ref)) * 100 / 2039, 4), round(max(get_tflops(data_ref)) * 100 / 312, 4))
        # # if efficiency >= 100 or spdup >= 10:
        # #     assert False, f"{path_gen.split('/')[-1]} test failed!"
        # if efficiency1 > efficiency:
    ...
    def test_opt_correctness(self, code, filename, tmp_dir, save_scripts=True, exe_dir="pass_exe",gpu_id=0):
        """
        Runs a given Python script on a specified GPU.
        """
        os.makedirs(exe_dir, exist_ok=True)
        call_status, exec_status, call_stdout, call_stderr, exe_stdout, exe_stderr = code_call_exec_success_allclose(code=code, fname=filename, temp_root=tmp_dir, py_folder=self.py_folder, gpu_id=gpu_id)
        pass_call = False
        pass_exe = False
